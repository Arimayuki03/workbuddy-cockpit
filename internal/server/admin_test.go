...
func TestAdminDisabledByDefault(t *testing.T) {
	...
	h := NewHandler(Config{Pool: p, APIKey: "k"}) // Admin.Enabled 零值 false
...
func TestAdminRequiresSameAPIKey(t *testing.T) {
	p := testPoolWith(&auth.Auth{UID: "u1"})
	h := NewHandler(Config{Pool: p, APIKey: "secret", Admin: AdminConfig{Enabled: true}})
...
func TestAdminDisableEnableRoundTrip(t *testing.T) {
	p := testPoolWith(&auth.Auth{UID: "u1"})
	h := NewHandler(Config{Pool: p, Admin: AdminConfig{Enabled: true}})
...
// ---- 上游运维端点测试辅助（admin_test 合并：doAdmin/adminResp 来自上游版） ----

// adminResp 与 adminAccountState 同构（测试侧独立声明，避免测试跟着实现改字段名）。
