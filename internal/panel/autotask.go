...
var mpTaskCodes = map[string]bool{
	...
	// Tasks_4..7 存在性已由 panel 分支实测（accept 返回 prerequisite not met 链式
	// 依赖；Tasks_8 task not found，链条封顶 7 环）。链条每日零点解锁一环
	// （task locked until 次日），判据为 issue #42 描述 + 事件形状预置，
	// 解锁后逐个实测校正。
...
// （panel 分支 4f08798 实测两账号 +300c+5e，重跑幂等）。
...
// 事件两套口径勿混；panel 分支 4f08798 实测上报即 completed，claim +200c+5e）。
// 专家 id 必须是市场真实 ex_ id（空 id 服务端不入账）→ **accept 之前**先解析市场
// 列表：拉不到就整任务不动作，避免留下「已登记未上报」的半程态。复用既有
// MarketExpertList（expert_5 任务同源，实测可用）。
func runMiniExpert(p *Panel, a *auth.Auth) (string, error) {
	...
	target := t.Target
	if target <= 0 {
		target = 1 // 未 accept 的 mp 任务 progress 为 null，target 兜底
	}
	// 已达标（含 completed 未领）：直接领奖。
	if t.Current >= target || t.AcceptStatus == "completed" {
		credit, energy, err := p.cfg.Upstream.ClaimRewardMP(a, code)
...
// runMPMiniChatTask 按差额补报——panel 称 target=10，以解锁后实际下发为准。
...
// primary：mp 对话事件带 requestModelId/requestModelName=glm-5.2（panel 实测
// 发射点形状）；fallback：PC 域模型活跃上报（Model_chat_GLM5.2 同源）。
...
// 实测点亮）；fallback：mp 指纹灵感事件组（MiniPlaybookEvents）。
