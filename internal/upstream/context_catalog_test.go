...
// TestContextWindowListingV4RemoteWins 上游动态值（maxInputTokens>0）为权威：
// 无视知识表直接透出（包括与知识表不同的值）。
// （三级版 ContextWindowListing 已被 V4 取代删除，测试迁移到 V4 等价断言。）
func TestContextWindowListingV4RemoteWins(t *testing.T) {
	resetModelsDev()
	resetModelCatalog()
	...
	// 远端小值也权威——不「纠正」上游。
	if got := ContextWindowListingV4("glm-5.2", 32000, nil); got != 32000 {
		t.Errorf("small remote: context_length=%d want 32000 (remote authoritative)", got)
	}
}

// TestContextWindowListingV4KnowledgeTable 知识表命中（第 2 级）：远端零值 →
// 按模型补齐真实量级，不再透出假 131072。
func TestContextWindowListingV4KnowledgeTable(t *testing.T) {
	cases := []struct {
		model, source string
		want          int64
	}{
	...
	for _, c := range cases {
		if got := ContextWindowListingV4(c.model, 0, nil); got != c.want {
			t.Errorf("%s (%s): context_length=%d want %d", c.model, c.source, got, c.want)
		}
	}
}

// TestContextWindowListingV4UnknownFallback1M 知识表也未收录 → 1M 兜底
// （宁可高估不低估：高估代价=客户端不截断、上游报错重试；低估代价=丢上下文）。
func TestContextWindowListingV4UnknownFallback1M(t *testing.T) {
	resetModelsDev()
	resetModelCatalog()
	if got := ContextWindowListingV4("totally-unknown-model", 0, nil); got != 1000000 {
		t.Errorf("unknown model: context_length=%d want 1000000 (1M fallback)", got)
	}
	if got := ContextWindowListingV4("", 0, nil); got != 1000000 {
		t.Errorf("empty model: context_length=%d want 1000000", got)
	}
}

// TestMaxOutputTokensListingV4 max_output_tokens 查找口径与 context_length 不同：
// 未知 → 省略（ok=false），没有 1M 兜底（输出上限无安全侧可估）。
// （三级版 MaxOutputTokensListing 已被 V4 取代删除，测试迁移到 V4 等价断言。）
func TestMaxOutputTokensListingV4(t *testing.T) {
	resetModelsDev()
	resetModelCatalog()
	// remote 权威。
	if got, ok := MaxOutputTokensListingV4("glm-5.2", 64000, nil); !ok || got != 64000 {
		t.Errorf("remote: max_output_tokens=%d,%v want 64000,true", got, ok)
	}
	// 知识表命中。
	if got, ok := MaxOutputTokensListingV4("deepseek-v4-pro", 0, nil); !ok || got != 384000 {
		t.Errorf("table: max_output_tokens=%d,%v want 384000,true", got, ok)
	}
	// 知识表条目但输出上限未知（kimi-k2.8-preview/auto）→ 省略。
	if got, ok := MaxOutputTokensListingV4("auto", 0, nil); ok || got != 0 {
		t.Errorf("auto: max_output_tokens=%d,%v want 0,false (unknown → omit)", got, ok)
	}
	// 完全未知 → 省略。
	if _, ok := MaxOutputTokensListingV4("totally-unknown-model", 0, nil); ok {
