...
func TestFetchGlobalModelInfosProbe(t *testing.T) {
	...
	// 名单路径共享同一探测缓存：零额外上游请求（v3-config-merge 后首探 = v3 双 UA + v2 三路）。
	...
	if len(calls) != 3 || !containsStr(calls, "/v3/config") || !containsStr(calls, "/v2/enterprises/personal/models") {
		t.Fatalf("probe calls=%v want [/v3/config x2 /v2/enterprises/personal/models] (cache shared)", calls)
	}
	hasHy3 := false
	...
	if len(calls) != 3 {
		t.Errorf("cache: probe calls=%d want 3 (second hit 1h cache)", len(calls))
	}
}
