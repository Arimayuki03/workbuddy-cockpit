...
func Aggregate(r io.Reader) (map[string]any, error) {
						...
						for _, ci := range ch {
							...
							if delta, ok := c["delta"].(map[string]any); ok {
								...
								if txt, ok := delta["content"].(string); ok {
									// 上游 appendContent 收敛「已取到正文」不变量到唯一写入点
									//（空串不置位 latch，语义与本地审计修复等价，取上游形态）。
									appendContent(txt)
								}
	...
	if usage != nil {
		...
		// 别名归一（usage.go）：缓存命中量在 details/扁平别名间取最大真值回写全部
		// 别名，防止严格按单一别名口径的下游把真实命中读成 0。
...
func normalizeFrame(obj map[string]any) map[string]any {
	...
	if u, ok := obj["usage"].(map[string]any); ok {
		// 末帧 usage 缓存别名归一（usage.go）：流式与聚合同口径，防止下游按
		// 单一别名读到 0（details 有真值、扁平别名留 0 的上游实测形态）。
