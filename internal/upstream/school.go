...
// 有效对话」的判据事件：mp 指纹 expert_actual_use（吸收 panel 分支 4f08798，上游
// task_runner 实测 2026-09-23：上报即 completed，claim +200c+5e）。与 school 域的
// SchoolExpertUseEvents 是**两套口径**，勿照抄：
...
//
// expertID 必须是专家市场真实 ex_ id（MarketExpertList，expert_5 任务同源），
// 空 id 服务端不入账。
...
// 载体，吸收 panel 分支 341f919）：小程序 chat_request_send 真实发射点带
...
// 载体，吸收 panel 分支 341f919；形状对齐小程序 playbook_cta_click →
// playbook_prompt_send 发射点）。panel 称该任务疑为 PC 口径（+500c+5e）——PC 序列
...
func MiniPlaybookEvents(caseID, caseName string) []map[string]any {
	...
	send := map[string]any{
		...
		"conversationId": growthClientToken("wb2api-mp-pb"),
		"extVersion":     "2.2.8",
	}
	for k, v := range base {
		send[k] = v
