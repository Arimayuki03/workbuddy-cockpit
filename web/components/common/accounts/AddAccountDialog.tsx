...
/**
 * 国际版可选地区（与 Go 侧 loginRegions 白名单一致）。
    ...
    try {
      ...
      const tick = async () => {
        ...
        try {
          ...
          failsRef.current = 0;
          if (res.done) {
            // 完成：凭证已落盘并热加载进池（panel loginPoll 语义）
            stopPoll();
            setPhase('success');
            const accountName = res.nickname || res.uid || '';
            setMessage(t('addAccount.success', {name: accountName}));
            notify.ok(
          ...
          // 未完成（done=false，"login ing"）：继续轮询。
          // 过期的 state 会以 404 + "unknown or expired state" 报错，
          // 落到 catch 里按连续失败处理。
        } catch (e) {
          // 连续失败到阈值就报出来：轮询出错（网络抖动 / state 过期）以前会被
      ...
      // **切回标签页立即补一次**：用户点授权链接会跳到新标签完成登录，
      // 原标签进入后台被节流；没有这一句，他切回来看见的仍是切走前那一帧。
