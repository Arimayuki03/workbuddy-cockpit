...
- **全量审计** —— 每次调用记录密钥、IP、模型、状态码、延迟、Token 消耗与**实际扣费**
  （取自上游 `usage.credit`；上游未返回时显示 `—`，与「扣了 0」区分开）

### 可视化设置
- **定时任务**：签到 / 猫猫旅行 / 活跃上报 / 保活四类任务各自独立开关与执行时刻
  （整点数组，如 `9, 21`），附白话说明，不再手改 JSON
- **系统提示词**：`prompt.mode` 可在 `custom`（网关提示词替换客户端 system）
  与 `passthrough`（透传客户端 system）之间切换——**依赖自带 system prompt
  的应用应选 passthrough**；另可指定自定义提示词文件。该分组带显式风险警示
