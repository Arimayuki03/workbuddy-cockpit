...
**分工边界**（与上游的定位一致）：上游只做**上游网关**（对接 CodeBuddy、暴露 OpenAI
Chat 协议），本项目是它的**独立前端面板**，两者解耦部署。因此本项目的边界是：

- **只做面板与 OpenAI 协议转发** —— 不把 `/v1` 转成 Anthropic Messages、Gemini
  等其他下游协议（那属于下游网关的职责）
- **不替代上游** —— 账号调度、令牌刷新、限流熔断全部仍由 workbuddy2api 完成，
  本项目只读它的 `/status` 并调用它的接口
- 上游在它的 README 中把本项目列为「社区前端面板」之一，我们与其保持版本
  松耦合：上游新增能力时本项目跟随适配，但**不要求上游为面板改任何代码**
