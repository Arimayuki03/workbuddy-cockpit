...
<p align="center">
  ...
  Web 面板 · OAuth 浏览器登录 · 账号池轮转 · 熔断与冷却 · 会话粘性 · 定时签到 / 活跃 / 旅行 / 保活 · <b>成长任务一键完成（14/18）</b> · 流式 / 非流式
</p>
...
**全新账号一键全做完 ≈ +1750 credits +73 能量**，其中仅 3 个任务涉及真实对话（`Model_chat_GLM5.2` 一条、`expert_5`/`Expert_team_use_3` 各数条 fast-model 短对话），其余全部为行为事件上报，零对话消耗。

### 不可自动的 4 个

| 任务 | 原因 |
|---|---|
| `skill_1` | 判据疑似要求真实技能工具调用链，纯脚本无法伪造 |
| `Expert_lighthouse` | 需真实连接器授权并领取服务器 |
| `Expert_Philanthropy` | 真实捐款 |
| `black_cat` | 夜间 23:00–8:00 时段任务 ×3 天（可用活跃排程顺带完成） |
