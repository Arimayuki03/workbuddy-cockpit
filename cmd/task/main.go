// task.go — 定时任务手动一次性触发器。
//
// 与常驻服务的关系：两边是独立进程、共享同一 state.json 与 auths/，无互斥协议——
// 本工具跑完落盘的结果（credits/快过期桶）会被常驻服务 5s 周期 flusher 的旧内存
// 快照覆盖（反过来亦然）。与常驻服务的自动排程并行运行是安全的（幂等性由上游
// 端点与 scheduler 的防抖/已签到/已领取判定兜底），但「本工具的落盘状态」不能
// 作为常驻服务的权威状态来源——两边各自维护自己的运行态。
//
// 用法：
//
