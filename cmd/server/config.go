...
type Config struct {
	...
	// Admin /admin 本地管理 API（TrafficMonitor 插件配套 + 上游运维端点）。缺省关闭：
	// enabled=false（或整段缺失）时不注册任何 /admin 路由，行为与引入前逐位一致。
	// 开启后本仓库扩展的端点仅接受 loopback 来源 + 既有 api_key（见 internal/server/admin.go）；
	// 上游 /admin/accounts/{uid}/{disable,enable,revive} 运维端点鉴权与 /status 同源
	// （withAuth + 同一个 api_key，不另立管理密钥，issue #138/#118）。
	Admin struct {
